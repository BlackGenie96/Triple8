...
function addToPlay($con, $phone, $name, $number, $money_amount){
    ...
    $sql = "INSERT INTO play_gold(user_name,user_phone,play_time,play_num, user_user_id) VALUES ('$name', '$phone',CURRENT_TIMESTAMP, '$number', '$userId')";
...
function play($con, OneEight $oneEight, DoubleEight $doubleEight, TripleEight $tripleEight, $response, $name, $phone, $money){
   
    //first get the count for all three digits
    $countThree  = $tripleEight->getCount();

    //next get the count of the users in the play table in the database
    $sql = "SELECT * FROM play_premium";
    $query = mysqli_query($con, $sql);
    ...
    switch($countOfUsers){
       case $countThree:
            ...
            $ill = $tripleEight->getOtherNumbers();
            $hii = $tripleEight->getTrippleEight();
            $sam = $tripleEight->getThirdNumbers();

            addToPlay($con, $phone, $name, $ill.$hii.$sam, $money);

            $response["success"] = 1;
            $response["message"] = "Grand Prize Won: \nE 1 000.00";
           $response["number"] = $ill.$hii.$sam;
...
if(isset($_POST['play']) && isset($_POST['name']) && isset($_POST['phone']) && isset($_POST['money'])){
    ...
    play($con, $oneEight,$doubleEight,$tripleEight,$response, $name, $phone, $money);
}else{
    $response["success"] = 0;
