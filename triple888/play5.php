...
function addToPlay($con, $phone, $name, $number, $money_amount){
    ...
    $sql = "INSERT INTO play_premium(user_name,user_phone,play_time,play_num, user_user_id) VALUES ('$name', '$phone',CURRENT_TIMESTAMP, '$number', '$userId')";
...
function play($con, OneEight $oneEight, DoubleEight $doubleEight, TripleEight $tripleEight, $response, $name, $phone, $money){
    ...
    switch($countOfUsers){
        case $countTwo:
            ...
            $ichi = $doubleEight->getOtherNumbers();
            $nii = $doubleEight->getDoubleEight();
            $san = $doubleEight->getOtherNumbers();

            addToPlay($con, $phone, $name, $ichi.$nii.$san, $money);

           $response["number"] = $ichi.$nii.$san;
           $response["success"] = 1;
